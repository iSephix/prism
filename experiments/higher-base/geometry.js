...
// Isolated fork of the validated 0.3.4 registration engine, parameterized for PX.
// No stable implementation is modified or monkey-patched.
(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory(require('../../vendor/jsqr-locator.js'),require('../../src/codec.js'));else root.PrismXGeometry=factory(root.Prism19Locator,root.Prism19Core);})(globalThis,function(locator,stable){
return {create(layout,symbols,mask){const C={layout,resize:stable.resize},A={symbols,mask};
  'use strict';
  ...
  function quality(image, location) {
    return C.layout(location.dimension).pilots.reduce((sum, cell, i) =>
      sum + pilotCost(image, location, cell, i % A.symbols.length), 0) / (2 * A.symbols.length);
  }
  function alignPilots(image, location, deadline) {
    ...
    for (let i = 0; i < pilots.length; i++) {
      if (expired(deadline)) return null;
      const cell = pilots[i]; let best = pilotCost(image, location, cell, i % A.symbols.length), shift = [0, 0];
      for (let y = -3; y <= 3; y++) for (let x = -3; x <= 3; x++) {
        const cost = pilotCost(image, location, cell, i % A.symbols.length, x / 10, y / 10);
